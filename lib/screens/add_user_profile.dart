...
import 'package:not_a_taxi/screens/metamask.dart';
import 'dart:io';
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  final FocusNode focusNodeUPI = FocusNode();
                ...
                Padding(
                    ...
                    child: Column(
                      ...
                      context, const Text('Your First Name is required!'));
                        ...
                        TextFormField(
                          focusNode: focusNodeUPI,
                          controller: _upiController,
                          validator: (value) {
                            if (value!.isEmpty) {
                              CustomSnackBar(
                      context, const Text('Your UPI ID is required!'));
                            } else {
                              return null;
                            }
                          },
 ...
 submitAction(BuildContext context) {
    if(_fnameController.text.isEmpty)
      ...
      else if(_upiController.text.isEmpty)
      {
        CustomSnackBar(context, const Text('Your UPI ID is required!'));
      }
    else{
    DatabaseManager().createUserData(_fnameController.text, _snameController.text, _upiController.text, imgURL, userID);
    CustomSnackBar(context, const Text('Profile Registered!'));
    Timer(Duration(seconds: 1), () { // <-- Delay here
      setState(() {
        Navigator.push(context,
                            MaterialPageRoute(
                              builder: (context) => metaMask(),
                            ),);
