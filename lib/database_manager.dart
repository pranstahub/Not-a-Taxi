...
class DatabaseManager {
  ...
  final CollectionReference upi = FirebaseFirestore.instance.collection('upiIDs');
  ...
  Future<void> addUPI(String UPI, String uid) async {
    final response = await upi.doc(uid).get();
    if(response.exists) {
      return await upi.doc(uid).update({'UPI': UPI});
    }
    else {
      return await upi.doc(uid).set({'UPI': UPI});
    }
  }

  Future getUPI(String userid) async {
  final response = await upi.doc(userid).get();
  final result = await response.get("UPI");
  print(result);
    return result;
  }
  ...
  String fullName, String ph, String imgURL, String uid) async {
    return await profileList
      .doc(uid)
        .set({'First Name': firstName, 'Second Name': secondName, 
        'Full Name': fullName, 'Mobile': ph, 'PFP URL': imgURL});
  }
